...
export default function PrivateDashboard({ userData }: { userData: any }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 pt-24 pb-20 w-full min-h-screen">
      <motion.div 
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-6 mb-8"
      >
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleLogout} className="px-4 py-2 bg-red-500/10 text-red-600 rounded-full text-sm font-medium hover:bg-red-500/20 transition-colors">
            Logout
          </button>
        </div>
      </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800">
              <p className="text-sm leading-relaxed">{profile?.bio || 'No bio set. Update your bio through WhatsApp.'}</p>
            </div>
          </div>
          <div>
