...
export default function TradingPage() {
    ...
    useEffect(() => {
        ...
        if (candleDataRef.current.length > 0) {
            ...
            const mockVolumes: any[] = [];
            let currentPrice = selectedAsset?.price || 965000;

            for (let i = 50; i >= 0; i--) {
                const time = Math.floor((now.getTime() - i * 60000) / 1000) as UTCTimestamp;
            ...
            try {
                ...
                console.error('Set mock data error:', err);
            }
        }
        ...
        return () => {            window.removeEventListener('resize', handleResize);
            if (chartRef.current) {
                try {
                    ...
                    console.error('Cleanup remove error:', err);
                }
                chartRef.current = null;
                candleSeriesRef.current = null;
                volumeSeriesRef.current = null;
    ...
    const estimatedTotal = useMemo(() => {
        ...
        return amt * prc;
    }, [tradeAmount, limitPrice, selectedAsset?.price, orderType]);

    const fee = estimatedTotal * 0.001;

    const getAssetColor = (id: string | undefined) => {
        switch (id) {
            ...
            <div className="border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#181A20] px-4 py-3 flex items-center justify-between sticky top-0 z-30">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`p-1.5 rounded-lg ${getAssetColor(selectedAsset?.id)} bg-zinc-100 dark:bg-zinc-800`}>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                            <div className="flex items-baseline gap-3">
                                <div className={`text-3xl font-bold transition-colors duration-300 ${
                                    ...
                                    Rp {formatPrice(selectedAsset?.price)}
                                </div>
                ...
                <div className="lg:col-span-3 space-y-4">
                    <div className="bg-white dark:bg-[#181A20] rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                        ...
                        <div className="max-h-[180px] overflow-y-auto">
                            {Array.from({ length: 8 }).map((_, i) => {
                                const basePrice = selectedAsset?.price || 0;
                                ...
                                return (
                                    <div key={`ask-${i}`} className="flex justify-between px-3 py-1 text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer relative">
                                        ...
                                        <span className="text-[#f6465d] relative z-10">{formatPrice(price)}</span>
                    ...
                    <div className="bg-white dark:bg-[#181A20] rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                        ...
                        <div className="max-h-[200px] overflow-y-auto">
                            {lastTrades.length === 0 ? (
                                <div className="text-center py-4 text-zinc-500 text-xs">Belum ada transaksi</div>
                            ) : (
                                lastTrades.map((trade) => (
                                    <div key={trade.id} className="flex justify-between px-3 py-1.5 text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                        <span className={trade.side === 'buy' ? 'text-[#0ecb81]' : 'text-[#f6465d]'}>{formatPrice(trade.price)}</span>
                        ...
                        <div className="bg-white dark:bg-[#181A20] rounded-xl border border-zinc-200 dark:border-zinc-800 p-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {portfolio.filter((p) => p.amount > 0).map((p) => {
                                    ...
                                    const pnlPercent = p.buyPrice > 0 ? ((asset.price - p.buyPrice) / p.buyPrice) * 100 : 0;
                                    return (
